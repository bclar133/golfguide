...
(
  ...
  node["sport"="golf"]["name"](area.australia);
  way["sport"="golf"]["name"](area.australia);
  relation["sport"="golf"]["name"](area.australia);
);
out center tags;`;
...
async function main() {
  const elements = await fetchNationalElements();
  if (elements.length <= 100) {
    throw new Error('OpenStreetMap import failed; refusing to deploy preview fallback data.');
  }

  const generated = elements.map(fromOverpass).filter(Boolean);
  ...
  console.log(`Generated ${deduped.length} courses from ${elements.length} imported OSM features`);
}

async function fetchNationalElements() {
...
function fromOverpass(element) {
  ...
  const state = clean(normaliseState(tags['addr:state']) || inferState(lon, lat));
...
function normaliseState(value) {
  ...
  return aliases[text] || (states.includes(text) ? text : '');
}
