...
async function main() {
  try {
    ...
    writeCourses(deduped, '// Generated by scripts/build-pages-data.mjs from OpenStreetMap/Overpass state boundary imports plus verified supplements.');
    console.log(`Generated ${deduped.length} courses from ${imported.length} imported OSM features`);
  } catch (error) {
    console.warn(`OpenStreetMap import failed; reusing published course data. ${error && error.message ? error.message : error}`);
    const fallback = await fetchPublishedCourses();
    if (fallback.length <= 1000) {
      throw new Error(`Published fallback returned only ${fallback.length} courses.`);
    }
    writeCourses(fallback, '// Reused by scripts/build-pages-data.mjs from the last published GitHub Pages course data because OpenStreetMap/Overpass was unavailable.');
    console.log(`Reused ${fallback.length} courses from published fallback data`);
  }
}

function writeCourses(courses, header) {
...
async function fetchPublishedCourses() {
  ...
  if (!response.ok) throw new Error(`Published fallback returned HTTP ${response.status}`);
  const text = await response.text();
  const match = text.match(/window\.GOLF_COURSES\s*=\s*([\s\S]*);\s*$/);
  if (!match) throw new Error('Could not parse published fallback course data.');
